...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create_campaign'])) {
    if(isset($conn)){
        ...
        $insert_sql = "INSERT INTO campaigns (campaign_name, campaign_type, description, target_audience, budget, currency, start_date, end_date, assigned_to, deal_id, status) 
                       VALUES ('$campaign_name', '$campaign_type', '$description', '$target_audience', '$budget', '$currency', '$start_date', '$end_date', '$assigned_to', $deal_id_sql, '$status')";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_campaign'])) {
    if(isset($conn)){
        ...
        $deal_id_sql = $deal_id !== NULL ? "$deal_id" : "NULL";
        $update_sql = "UPDATE campaigns SET campaign_name='$campaign_name', campaign_type='$campaign_type', description='$description', target_audience='$target_audience', budget='$budget', currency='$currency', start_date='$start_date', end_date='$end_date', assigned_to='$assigned_to', deal_id=$deal_id_sql, status='$status' WHERE id='$id'";
...
$campaignTableRows = "";
if(isset($conn)){
    $campaigns_query = mysqli_query($conn, "SELECT c.*, d.deal_name FROM campaigns c LEFT JOIN deals d ON c.deal_id = d.id ORDER BY c.start_date DESC, c.id DESC");
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        .camp-btn-next, .camp-btn-submit { background-color: #2563eb; color: #fff; }
        .camp-btn-next:hover, .camp-btn-submit:hover { background-color: #1d4ed8; }
        .camp-btn-submit { background-color: #10b981; }
        .camp-btn-submit:hover { background-color: #059669; }
    </style>
</head>
<body>
    ...
    <div id="createCampaignModal" class="modal">
        <div class="modal-content">
            ...
            <form action="campaigns.php" method="POST" id="createCampaignForm">
                ...
                <!-- Step 3: Timeline & Audience -->
                <div class="camp-step-container" data-step="3">
                    <div class="form-grid">
                        <div class="form-group">
                            <label>Start Date <span style="color:red">*</span></label>
                            <input type="date" name="start_date" id="camp_start_date" required>
                        </div>
                        <div class="form-group">
                            <label>End Date <span style="color:red">*</span></label>
                            <input type="date" name="end_date" id="camp_end_date" required>
                        </div>
                        <div class="form-group full-width">
                ...
                <div class="camp-footer-nav">
                    ...
                    <button type="button" class="camp-btn-nav camp-btn-next" id="nextBtn" onclick="changeStep(1)">Next <i class="fa-solid fa-arrow-right"></i></button>
                    <button type="submit" name="create_campaign" class="camp-btn-nav camp-btn-submit" id="submitBtn" style="display:none;">Create Campaign <i class="fa-solid fa-check"></i></button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function openEditModal(campaign) {
            ...
            document.getElementById('edit_deal_id').value = campaign.deal_id || '';
        ...
        function updateProgress() {
            const progressSteps = document.querySelectorAll('.camp-progress-step');
        ...
        function validateStep(step) {
            let valid = true;
            const container = document.querySelector(`.camp-step-container[data-step="${step}"]`);

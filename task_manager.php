...
$toastMessage = "";
$toastType    = "";
$currentUser  = $_SESSION['name']; // যে লগইন আছে সে-ই "Assigned By"
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create_task'])) {
    if (isset($conn)) {
        ...
        $assigned_by = mysqli_real_escape_string($conn, $currentUser);
        ...
        $sql = "INSERT INTO tasks (title, description, assigned_to, assigned_by, priority, status, due_date)
                VALUES ('$title','$description','$assigned_to','$assigned_by','$priority','$status','$due_date')";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_task'])) {
    if (isset($conn)) {
        ...
        $sql = "UPDATE tasks SET title='$title', description='$description', assigned_to='$assigned_to',
                priority='$priority', status='$status', due_date='$due_date', updated_at=NOW() WHERE id='$id'";
        if (mysqli_query($conn, $sql)) {
...
$assigneeOptions = "";
if (isset($conn)) {
    $uq = mysqli_query($conn, "SELECT username, name FROM users ORDER BY name ASC");
...
if (isset($conn)) {
    $cr = mysqli_query($conn, "SELECT status, COUNT(*) as cnt FROM tasks GROUP BY status");
    if ($cr) {
    ...
    $today = date('Y-m-d');
    // Robust overdue: due_date must be a valid real date (not 0000-00-00, not NULL, not empty)
...
if (isset($conn)) {
    @mysqli_query($conn, "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS assigned_by VARCHAR(100) DEFAULT NULL");
    @mysqli_query($conn, "ALTER TABLE tasks ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP");
    $tq = mysqli_query($conn, "SELECT * FROM tasks ORDER BY id DESC");
    if ($tq && mysqli_num_rows($tq) > 0) {
        while ($row = mysqli_fetch_assoc($tq)) {
            ...
            $overdueStyle = $isOverdue ? ' style="background:#fff5f5!important;border-left:3px solid #ef4444;"' : '';
...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Inter',sans-serif; }
        ...
        body.dark-mode .custom-table tbody tr:nth-child(even) { background:#1e293b; }
...
<?php $activePage = 'task_manager'; include 'sidebar.php'; ?>
                        ...
                        <div class="multi-select-wrapper" id="multiSelectWrapper">
                            ...
                            <div class="multi-dropdown" id="multiDropdown">
                                ...
                                <div class="multi-options" id="multiOptions">
                                    <?php
                                    if (isset($conn)) {
                                        $uq2 = mysqli_query($conn, "SELECT username, name FROM users ORDER BY name ASC");
                                        while ($u = mysqli_fetch_assoc($uq2)) {
                    ...
                    <div class="multi-select-wrapper" id="editMultiSelectWrapper">
                        ...
                        <div class="multi-dropdown" id="editMultiDropdown">
                            ...
                            <div class="multi-options" id="editMultiOptions">
                                <?php
                                if (isset($conn)) {
                                    $uq3 = mysqli_query($conn, "SELECT username, name FROM users ORDER BY name ASC");
                                    while ($u = mysqli_fetch_assoc($uq3)) {
...
<script>
    ...
    function toggleMultiDropdown() {
        ...
        document.getElementById('editMultiDropdown').classList.remove('open');
        document.getElementById('editMultiSelectBox').classList.remove('open');
        document.getElementById('editMultiArrow').classList.remove('open');
    ...
    function toggleEditMultiDropdown() {
        ...
        document.getElementById('multiSelectBox').classList.remove('open');
        document.getElementById('multiArrow').classList.remove('open');
        dd.classList.toggle('open', !isOpen);
        box.classList.toggle('open', !isOpen);
        arr.classList.toggle('open', !isOpen);
    }
    ...
    document.addEventListener('click', function(e) {
        ...
        if (!document.getElementById('editMultiSelectWrapper').contains(e.target)) {
            document.getElementById('editMultiDropdown').classList.remove('open');
            document.getElementById('editMultiSelectBox').classList.remove('open');
            document.getElementById('editMultiArrow').classList.remove('open');
        }

...
if (isset($conn)) {
    ...
    $today = date('Y-m-d');
    $due_r = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM tasks WHERE due_date = '$today' AND status != 'Completed'");
    if ($due_r) $dueTodayCount = mysqli_fetch_assoc($due_r)['cnt'];
    $total_due_r = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM tasks WHERE due_date IS NOT NULL AND due_date != '' AND due_date != '0000-00-00' AND status != 'Completed'");
    $totalDueCount = ($total_due_r) ? (int)mysqli_fetch_assoc($total_due_r)['cnt'] : 0;
}

$taskTableRows = "";
if(isset($conn)){
    $tasks_query = mysqli_query($conn, "SELECT * FROM tasks ORDER BY id DESC");
...
<body>
    ...
    <div id="toastBox">
        ...
        <?php $activePage = 'user_tasks'; include 'sidebar.php'; ?>
